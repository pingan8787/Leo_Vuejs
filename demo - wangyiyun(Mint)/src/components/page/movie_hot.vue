...
<div>
  <div v-infinite-scroll="loadMore" infinite-scroll-disabled="loading" infinite-scroll-distance="50"> 
      <mt-cell class="movie_cell" v-for="list in list_data" :href="list.alt" :key="list.id">
          <span class="movie_cell_title">{{list.title}}{{list.id}}</span>
...
export default {
    ...
    methods:{
        loadMoreTop(){
            ...
            this.$ajax({
                methods:"GET",
                url:'https://bird.ioliu.cn/v1/?url=https://api.douban.com/v2/movie/in_theaters?count='+this.count+'&start='+this.start
            })
            .then((res)=>{
